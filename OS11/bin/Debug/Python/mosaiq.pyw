...
docFolder= ctypes.create_unicode_buffer(ctypes.wintypes.MAX_PATH)
ctypes.windll.shell32.SHGetFolderPathW(0, CSIDL_PERSONAL, 0, SHGFP_TYPE_CURRENT, docFolder)

fptr=open(sys.argv[2]+"\\..\\Config\\config.json")
params=json.load(fptr)
####################
# Global Variables 
variancePercentage = 0.5
meanPercentage = 0.5
arcLengthmultiplier = float(params["arcLengthMultiplier"])
minVarianceThreshold = 0 # 1% of the max images height, width
noiseReduction = int(params["noiseReduction"])  # 0 means false 1 true
scaleFactor = float(params["scaleFactor"])
meanDiffThreshold =0
maxVariance = 0
approxPolyparameter = float(params["approxPolyparameter"])
multiEdge = int(params["multiEdge"])     # 1 for including 0 for not 
varianceMultiplier = 3
backgroundThreshold = float(params["backgroundThreshold"])
#####################
...
def getAngle(p1,p2,p3):
		dotProduct = (p2[0] - p1[0]) * (p3[0] - p2[0]) + (p2[1] - p1[1]) * (p3[1] - p2[1])
		magnitude = math.sqrt((p1[1]-p2[1])**2+(p2[0]-p1[0])**2) * math.sqrt((p3[1]-p2[1])**2+(p3[0]-p1[0])**2)
		if(((p1[0] - p2[0]) * (p3[1] - p2[1]) - (p3[0] - p2[0]) * (p1[1] - p2[1])) > 0):
			direction = -1
		else :
			direction = 1

		factor = dotProduct/magnitude
		if(factor > 1.0):
			factor = 1.0
		elif(factor < -1.0):
			factor = -1.0
		return direction * math.acos(factor)
...
def processImage(im,filename):
	...
	if maxi>127:
		thresh = maxi-backgroundThreshold
	...
	if(multiEdge):
		for i in range(lenBoundary):
			point1 = contoursList[indices[i]]
			point2 = contoursList[indices[(i+1)%lenBoundary]]
			point3 = contoursList[indices[(i+2)%lenBoundary]]		
		...
		for imagej in range(len(images)):
			...
			for i in range(len(edgeEnvelopesi)):
				...
				for j in range(len(edgeEnvelopesj)):
					...
					#NEEDED = NEEDED*(1+arcLengthmultiplier*(math.fabs(dist1-dist2)/min(dist1, dist2)))
					global varianceMultiplier
					NEEDED = NEEDED/(1+ varianceMultiplier*math.sqrt(minVariance/maxVariance))
...
def writeOutput(images,initialCount):
	writeProgress(float(initialCount-len(images))/float(initialCount-1))
	directoryPath = sys.argv[2]
	files=[]
	data={}
	data["nFiles"]=len(images)
	for i in range(len(images)):		
		cv2.imwrite(directoryPath+'output'+str(i+1)+'.jpg', images[i].original)
		files.append(directoryPath+'output'+str(i+1)+'.jpg')
	data["files"]=files
	
	if len(images) == 1 :
		data["status"]= 'success'
		data["msg"]  = 'Succesfully Reconstructed!'
	elif len(images) < initialCount :
		data["status"]= 'partial'
		data["msg"]  = 'Partially Reconstructed! Tune the parameters and try again'
	else:
		data["status"]= 'failure'
		data["msg"]  = 'Unable to reconstruct! Tune the parameters and try again'	
	
	data["completion"] = str(float(initialCount-len(images))/float(initialCount-1))
	
	fptr=open(sys.argv[2]+"\\output.json","w")
...
def writeProgress(percentage):
	fptr = open(sys.argv[2]+"\\progress","w")
	fptr.write(str(percentage))
	fptr.close()
...
def main():
	...
	while len(images)>1:
		writeProgress(float(completion)/float(imageCount-1))
		completion +=1
		straights = []
		for image in images:
